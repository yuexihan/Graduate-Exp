...
if os.path.exists('ieee_process_for_lda.json'):
    processed = json.load(open('ieee_process_for_lda.json'))
else:
    processed = []
    with open('../data/IeeeAfter2010.csv', 'rb') as f:
        f.readline()
        reader = csv.reader(f)
        for paper_id, title, abstract, category in tqdm(reader):
    ...
    with open('ieee_process_for_lda.json', 'w') as f:
...
def init_word_index():
    ...
    with open('re_index_to_word.json', 'w') as f:
        json.dump(index_to_word, f)
    with open('re_word_to_index.json', 'w') as f:
        json.dump(word_to_index, f)


def load_word_index():
    with open('re_index_to_word.json') as f:
        index_to_word = json.load(f)
    with open('re_word_to_index.json') as f:
...
def main():
    ...
    if os.path.exists('re_index_to_word.json') and os.path.exists('re_word_to_index.json'):
