...
flags.DEFINE_string("train_dir", "FASTTEXT_train/", "Directory where to write event logs and checkpoint.")
...
#		self.sess = tf.Session(config=tf.ConfigProto(log_device_placement=True))
    ...
    def hidden(self, x):
        with tf.device("/cpu:0"):
            ...
            hidden = tf.reduce_mean(embed, axis=0)
        return hidden
