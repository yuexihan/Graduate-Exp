...
class Loader(object):
    ...
    def get_references(self):
        ...
        new_references = set()
        for citing, cited in references:
            indexed_references[citing].add(cited)
            indexed_references[cited].add(citing)
            if citing > cited:
                new_references.add((citing, cited))
            elif citing < cited:
                new_references.add((cited, citing))
        for citing, cited in references:
            for a in indexed_references[citing]:
                for b in indexed_references[cited]:
                    if a > b:
                        new_references.add((a, b))
                    elif a < b:
                        new_references.add((b, a))
        print 'first + seconde order reference: %d' % len(new_references)
